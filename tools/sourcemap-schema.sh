...
schema_json=${SOURCEMAP_SCHEMA_JSON:-"$repo_dir/schemas/sourcemap-v3.json"}
schema_artifact=${SOURCEMAP_SCHEMA_ARTIFACT:-"$repo_dir/schemas/sourcemap-v3.isf"}
...
usage() {
    cat >&2 <<EOF
usage: tools/sourcemap-schema.sh [generate|check|all]

Environment overrides:
  SOURCEMAP_SCHEMA_JSON       human-authored JSON (default: schemas/sourcemap-v3.json)
  SOURCEMAP_SCHEMA_ARTIFACT   generated artifact (default: schemas/sourcemap-v3.isf)
...
check() {
    ...
    run_isoc hash "$schema_artifact" --type SourceMap
    run_isoc layout "$schema_artifact" --type SourceMap
}
