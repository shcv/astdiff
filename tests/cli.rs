...
#[test]
fn source_map_cli_preserves_ambiguity() {
    let directory = tempdir().unwrap();
    let map = directory.path().join("synthetic.map");
    std::fs::write(
        &map,
...
#[test]
fn canonical_pretty_output_preserves_function_syntax_and_mapping_round_trips() {
    let directory = tempdir().unwrap();
    let input = directory.path().join("input.js");
    std::fs::write(&input, "const holder = async function named(x=1,...rest){return x ? named(x-1,...rest) : rest.length;};").unwrap();
    let output = command()
        .arg("canon")
        .arg(&input)
        .arg("--pretty")
    ...
    assert!(canonical.contains("async function fn_"), "{canonical}");
    assert!(canonical.contains("= 1"), "{canonical}");
    assert!(canonical.contains("..."), "{canonical}");
    let mut parser = astdiff::parser::JsParser::new().unwrap();
    ...
    let map_path = directory.path().join("names.txt");
    std::fs::write(&map_path, mapping.stdout).unwrap();
    let applied = command()
        .arg("canon")
        .arg(&input)
        .arg("--map")
    ...
    assert_eq!(applied.stdout, canonical.as_bytes());
}
