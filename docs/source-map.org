...
# Persist and query a regular map through the verified positioned sidecar.
...
Regular maps can be persisted in the separate
=astdiff.sourcemap.v3.positioned= Isoform =.astsm= sidecar without changing
=astdiff.analysis.v1=.  Its envelope binds the exact layout hash, raw-map
digest, normalized payload digest, and generated-source digest.  After one
bounded full verification pass, a line directory provides O(1) line slicing
and lookup uses binary search within or before the requested line.  Indexed
maps remain queryable in memory but are rejected by this sidecar profile until
a canonical section-table representation is defined.  A reverse
original-to-generated index is also not materialized yet.
